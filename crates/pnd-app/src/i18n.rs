...
pub static ENGLISH: Text = Text {
    ...
    settings_poesessid_hint: "Encrypted with your Windows account and stored in settings.json on this machine — only this user on this PC can read it back. Needed only for live search and hideout travel; polling works without it.",
...
pub static SIMPLIFIED_CHINESE: Text = Text {
    ...
    settings_poesessid_hint: "用你的 Windows 账户加密后存在本机的 settings.json 里,换台机器或换个用户都解不开。只有 Live 秒推和去藏身处需要它,轮询没有它照样跑。",

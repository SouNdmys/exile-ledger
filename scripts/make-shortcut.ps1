# 在桌面放一个「POE Ninja Data」快捷方式,指向 release 版的 exe。
#
...
#   powershell -ExecutionPolicy Bypass -File scripts\make-shortcut.ps1 -ExePath D:\别处\pnd-app.exe
...
# 路径都一样。
if (-not $ExePath) {
    $ExePath = Join-Path (Split-Path -Parent $PSScriptRoot) 'target\release\pnd-app.exe'
}
if (-not $ShortcutPath) {
    $ShortcutPath = Join-Path ([Environment]::GetFolderPath('Desktop')) 'POE Ninja Data.lnk'
}
...
$lnk.Description = 'POE Ninja Data - unique price watcher + poe.ninja meta'
$lnk.Save()
